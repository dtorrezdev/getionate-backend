 ...
 */
package com.dtorrez.main.controllers;
...
import com.dtorrez.main.security.services.ActiveDirectoryService;
import com.dtorrez.main.common.exceptions.ApiException;
import com.dtorrez.main.common.exceptions.LdapContextException;
import com.dtorrez.main.controllers.template.GenericControler;
import com.dtorrez.main.controllers.template.ICrudControler;
...
import com.dtorrez.main.controllers.dto.UsuarioRequest;
import com.dtorrez.main.controllers.dto.UsuarioResponse;
import com.dtorrez.main.controllers.dto.PageResponse;
...
import com.dtorrez.main.services.LoggerWeb;
import com.dtorrez.main.validators.UsuarioValidator;
...
public class UsuarioControler extends GenericControler
        implements ICrudControler<UsuarioRequest, UsuarioResponse, String> {
        ...
        try {
            ...
            Page<UsuarioResponse> out = repository.filter(
                    ...
                    .map(model -> {
                        ...
                        usuarioResponse.setEstado(model.getEstado() == UsuarioEstado.HABILITADO
                                ? UsuarioEstado.HABILITADO_VALOR
    ...
    @Override
    public ResponseEntity<UsuarioResponse> get(String token, String ipClient, String form, String id)
            throws ApiException {
        /*
         * ipClient = obtenerIp(ipClient);
         * 
         ...
         * if (model.isPresent()) {
         * ResponseEntity<UsuarioResponse> out =
         * ResponseEntity.ok().body(ConvercionUtil.convertToObject(model.get(),
    ...
    public ResponseEntity<UsuarioResponse> create(String token, String tenantId, String ipClient, String form,
        ...
        try {
            ...
            if (result.hasErrors()) {
                ...
                // Acciones.USUARIO_CREAR, null, map);
            ...
            if (request.getTipo().equals(TipoAutenticacion.LDAP.getId())) {
                try {
                    name = new ActiveDirectoryService(parametroService)
                            .getNombreCompleto(request.getNombreUsuario().trim());
                } catch (LdapContextException ex) {
                    // logWeb.error(obtenerNombreUsuario(), Apps.TRAZABILIDAD_SISTEMA,
                    // Acciones.USUARIO_CREAR, "Error de conexion ldap " + ex.getMessage(), ex);
                    result.rejectValue("nombreUsuario", "field.nombreUsuario",
                            "Fallo con la conexión con active directory.");
                    map.put(TiposComunes.ERROR, obtenerErrores(result));
                    // bitacoraService.guardarBitacora(token, ipClient, form,
                    // Acciones.USUARIO_CREAR, null, map);
            ...
            int dias = ((BigDecimal) parametroService.getParamVal(Parametro.DelSistema.BLOQUEO_USUARIOS_DIAS.name()))
                    .intValue();
            Date fechaActualizacion = sumarDiasAFecha(new Date(), dias);

            // OAC LDAP
            /*
             * Usuario model = repository.save(new Usuario(null, request.getNombreUsuario(),
             * name,
             * (String) parametroService.getParamVal(ParametroID.CONTRASENA_POR_DEFECTO),
             * (request.getTipo() == UsuarioTipo.USUARIO_ACTIVE_DIRECTORY ?
             * UsuarioTipo.USUARIO_ACTIVE_DIRECTORY : UsuarioTipo.USUARIO_NORMAL),
             * (short) 0, null, rolRepository.findById(request.getRolId()).get(),
             * UsuarioEstado.HABILITADO,
             * fechaActualizacion));
             */
            String pass = (request.getTipo().equals(TipoAutenticacion.LDAP.getId()))
                    ? (String) parametroService.getParamVal(Parametro.DelSistema.CONTRASENA_POR_DEFECTO.name())
                    : request.getPass();
            // passwordEncoder
            ...
            final Long logSistemaId = logWeb.error(obtenerNombreUsuario(), Apps.TRAZABILIDAD_SISTEMA,
                    Acciones.USUARIO_CREAR, mensajeError, e);
            map.put(TiposComunes.MENSAJE_ERROR, mensajeError);
    ...
    public ResponseEntity<UsuarioResponse> update(String token, String ipClient, String form,
        ...
        try {
            ...
            if (result.hasErrors()) {
                map.put(TiposComunes.ERROR, obtenerErrores(result));
                // bitacoraService.guardarBitacora(token, ipClient, form,
                // Acciones.USUARIO_MODIFICAR, null, map);
                throw new ApiException(result, "Errores en la validacion");
            }
            ...
            if (request.getTipo().equals(TipoAutenticacion.LDAP.getId())) {
                try {
                    model.setNombreCompleto(
                            new ActiveDirectoryService(parametroService).getNombreCompleto(request.getNombreUsuario()));
                } catch (LdapContextException ex) {
                    // logWeb.error(obtenerNombreUsuario(), Apps.TRAZABILIDAD_SISTEMA,
                    // Acciones.USUARIO_MODIFICAR, "Error de conexion ldap " + ex.getMessage(), ex);
                    result.rejectValue("nombreUsuario", "field.nombreUsuario",
                            "Fallo con la conexión con active directory.");
                    map.put(TiposComunes.ERROR, obtenerErrores(result));
                    // bitacoraService.guardarBitacora(token, ipClient, form,
            ...
            ResponseEntity<UsuarioResponse> out = ResponseEntity.ok()
                    .body(ConvercionUtil.convertToObject(model, UsuarioResponse.class));
    ...
    public ResponseEntity<?> delete(String token, String ipClient, String form,
         ...
         * 
         * bitacoraService.guardarBitacora(token, ipClient, form, "Se elimino:" +
         * model);
         * 
    ...
    public ResponseEntity<?> resetearContrasena(@RequestHeader(value = JwtTokenUtil.KEY_TOKEN) String token,
        ...
        try {
            ...
            final Long logSistemaId = logWeb.error(obtenerNombreUsuario(), Apps.TRAZABILIDAD_SISTEMA,
                    Acciones.USUARIO_RESETEAR_CONTRASENA, mensajeError, e);
            map.put(TiposComunes.MENSAJE_ERROR, mensajeError);
            bitacoraService.guardarBitacora(token, ipClient, form, Acciones.USUARIO_RESETEAR_CONTRASENA, map, mapNuevo,
                    logSistemaId);
    ...
    ResponseEntity<?> deshabilitar(@RequestHeader(value = JwtTokenUtil.KEY_TOKEN) String token,
        ...
        try {
            ...
            final Long logSistemaId = logWeb.error(obtenerNombreUsuario(), Apps.TRAZABILIDAD_SISTEMA,
                    Acciones.USUARIO_INHABILITAR, mensajeError, e);
            map.put(TiposComunes.MENSAJE_ERROR, mensajeError);
            bitacoraService.guardarBitacora(token, ipClient, form, Acciones.USUARIO_INHABILITAR, map, mapNuevo,
                    logSistemaId);
    ...
    ResponseEntity<?> habilitar(@RequestHeader(value = JwtTokenUtil.KEY_TOKEN) String token,
        ...
        try {
            ...
            final Long logSistemaId = logWeb.error(obtenerNombreUsuario(), Apps.TRAZABILIDAD_SISTEMA,
                    Acciones.USUARIO_HABILITAR, mensajeError, e);
            map.put(TiposComunes.MENSAJE_ERROR, mensajeError);
            bitacoraService.guardarBitacora(token, ipClient, form, Acciones.USUARIO_HABILITAR, map, mapNuevo,
                    logSistemaId);
