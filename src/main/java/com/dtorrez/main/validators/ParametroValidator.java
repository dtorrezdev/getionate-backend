...
import com.dtorrez.main.controllers.dto.ParametroRequest;
...
public class ParametroValidator extends GlobalValidator {
    ...
    public void validate(ParametroRequest input, Long id, Errors errors) {
         ...
         * //log.error("valido: " +
         * org.quartz.CronExpression.isValidExpression(input.getValor()));
         * if (!org.quartz.CronExpression.isValidExpression(input.getValor())) {
         * errors.rejectValue("valor", "field.valor",
         * "Expresion Cron Quartz incorrecta.");
        ...
        if (!input.getNombre()
                .matches(parametroService
                        .getParametroByNombre(
                                com.micrium.bd.access.enuns.Parametro.DelSistema.EXPRESION_REGULAR_GENERAL.name())
                        .getValor())) {
            errors.rejectValue("nombre", "field.nombre",
                    parametroService
                            .getParametroByNombre(
                                    com.micrium.bd.access.enuns.Parametro.DelSistema.MENSAJE_VALIDACION_GENERAL.name())
                            .getValor());
        }
